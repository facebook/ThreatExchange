...
from hmalib.common.image_sources import (
    URLImageSource,
    S3BucketImageSource,
    S3LocalImageSource,
)
...
@dataclass
class URLImageSubmissionMessage(ImageSubmission):
    """
    ...
    def get_bytes(self) -> bytes:
        return URLImageSource().get_image_bytes(self.url)


@dataclass
class S3ImageSubmission(ImageSubmission):
    """
    ...
    def get_bytes(self) -> bytes:
        return S3BucketImageSource(self.bucket, self.image_prefix).get_image_bytes(
            self.content_id
        )


@dataclass
class S3ImageSubmissionBatchMessage(ImageSubmission):
    """
    ...
    def from_sqs_message(
        ...
        for s3_record in d["Records"]:
            ...
            result.append(S3ImageSubmission(content_id, bucket_name, key, image_prefix))
    ...
    def could_be(cls, d: dict) -> bool:
        ...
        return (
            "Records" in d
            and len(d["Records"]) > 0
            and "s3" in d["Records"][0]
            and not S3LocalImageSubmissionBatchMessage.could_be(d)
        )


@dataclass
class S3LocalImageSubmission(ImageSubmission):
    """
    S3 -> Lambda batches events together. This represents one of the events. An
    `S3LocalImageSubmissionBatchMessage` event is emitted. Each batch has one or more
    of these objects.
    """
    ...
    def get_bytes(self) -> bytes:
        return S3LocalImageSource().get_image_bytes(self.content_id)
...
class S3LocalImageSubmissionBatchMessage:
    ...
    image_submissions: t.List[S3LocalImageSubmission]

    @classmethod
    def from_sqs_message(cls, d: dict) -> "S3LocalImageSubmissionBatchMessage":
        result = []
