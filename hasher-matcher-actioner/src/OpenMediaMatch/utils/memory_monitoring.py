...
from pympler import muppy, summary  # type: ignore[import-not-found]
...
class MemorySnapshot:
    ...
    tracemalloc_stats: t.Optional[t.Dict[str, float]] = (
        None  # Memory stats from tracemalloc
    )
...
class MemoryMonitor:
    def __init__(
        self,
        enable_faiss_monitoring: bool = True,
    ...
    def take_snapshot(self) -> MemorySnapshot:
        ...
        if self.enable_detailed_profiling:
            ...
            try:
                ...
                for stat in tm_snapshot.statistics("lineno")[:5]:
                    tracemalloc_stats[stat.traceback.format()[0]] = (
                        stat.size / 1024 / 1024
                    )  # Convert to MB
            except Exception as e:
    ...
    def log_snapshot(self, message: str = "") -> str:
        ...
        faiss_memory_str = (
            f"{snapshot.faiss_memory:.2f}"
            if snapshot.faiss_memory is not None
            else "N/A"
        )
        ...
        top_objects_str = (
            "\n    ".join(
                [f"{obj[0]}: {obj[1]/1024/1024:.2f} MB" for obj in snapshot.top_objects]
            )
            if snapshot.top_objects
            else "None"
        )
        ...
        tracemalloc_str = (
            "\n    ".join(
                [
                    f"{loc}: {size:.2f} MB"
                    for loc, size in snapshot.tracemalloc_stats.items()
                ]
            )
    ...
    def get_memory_trend(self) -> t.Dict[str, t.Optional[float]]:
        ...
        return {
            ...
            "faiss_memory_change": (
                (last.faiss_memory - first.faiss_memory)
                if last.faiss_memory is not None and first.faiss_memory is not None
                else None
            ),
    ...
    def log_memory_trend(self) -> str:
        ...
        faiss_change_str = (
            f"{trend['faiss_memory_change']:.2f}"
            if trend["faiss_memory_change"] is not None
            else "N/A"
        )
        return f"""
        \n=== Memory Usage Trend ===\n
