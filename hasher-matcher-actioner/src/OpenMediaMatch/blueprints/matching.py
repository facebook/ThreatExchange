...
class _SignalIndexInMemoryCache:
    ...
    @classmethod
    def get_initial(cls, signal_type: t.Type[SignalType], sec_old_before_stale: int) -> t.Self:
        return cls(
            signal_type,
...
def initiate_index_cache(app: Flask, scheduler: APScheduler | None) -> None:
    ...
    storage = get_storage()
    cache = {
        st.signal_type.get_name(): _SignalIndexInMemoryCache.get_initial(st.signal_type, int(app.config.get("INDEX_CACHE_MAX_STALE_SEC", 65)))
