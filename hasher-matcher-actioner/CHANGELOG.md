...
Entries are newest-first. All entries up to and including 1.1.7 (`hma-v1.1.7`) were reconstructed after the fact, generated by AI from the git commit history, so they are less detailed than entries written at release time. The section template used by each entry is at the [bottom of this file](#entry-template).
