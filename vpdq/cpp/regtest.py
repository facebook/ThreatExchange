...
def main():
    ...
    for file in os.listdir(inputVideoFolder):
        if file.endswith('.mp4'):
            subprocess.call(["./build/vpdq-hash-video", "-f", ffmpegPath, '-r', secondsPerHash, '-d', outputHashFolder,'-s', downsampleFrameDimension, '-i', inputVideoFolder+"/"+file])
