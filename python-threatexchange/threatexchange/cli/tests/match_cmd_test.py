...
class MatchCommandTest(ThreatExchangeCLIE2eTest):
    ...
    def test_multiple_prefixes(self):
        ...
        with tempfile.NamedTemporaryFile("a+") as fp:
            ...
            self.assert_cli_output(("-H", "photo", fp.name), "pdq 16 (Sample Signals) INVESTIGATION_SEED")
    ...
    def test_incorrect_valid_and_no_prefixes(self):
        ...
        with tempfile.NamedTemporaryFile("a+") as fp:
            ...
            fp.write(hash3)
            fp.seek(0)
            self.assert_cli_usage_error(("-H", "photo", fp.name), f"Error attempting to infer Signal Type: '{fakeprefix}' is not a valid Signal Type.")
