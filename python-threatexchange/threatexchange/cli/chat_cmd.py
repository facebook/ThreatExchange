...
class ChatCommand(command_base.Command):
    ...
    def __init__(
        ...
        input: str,
        show_all: bool = False,
        policy: str = "threatexchange/policy/basic_policy.md",
    ):
        self.input = input
    ...
    def init_argparse(cls, settings: CLISettings, ap: argparse.ArgumentParser) -> None:
        ...
        ap.add_argument(
            ...
            "--policy",
            type=Path,
            default=(Path("threatexchange/policy/basic_policy.md")),
            help="Path to policy file (default: threatexchange/policy/basic_policy.md)",
        )

    def execute(self, settings: CLISettings) -> None:
        # Get text from stdin, file, or literal
        print("Policy definitions:", self.policy)
        if self.input == "-":
            text = sys.stdin.read()
        else:
            try:
