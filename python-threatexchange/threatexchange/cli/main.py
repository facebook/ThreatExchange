...
from threatexchange import type_mapping
...
def _get_settings(
    ...
    signals = type_mapping.SignalTypeMapping(
    ...
    fetchers = type_mapping.FetcherMapping(base_apis + extensions.api_instances)
    state = CliState(list(fetchers.fetchers_by_name.values()), dir=dir)

    return (
        CLISettings(type_mapping.FunctionalityMapping(signals, fetchers, state), state),
        extensions,
    )
