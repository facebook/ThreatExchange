...
// ================================================================
// Step 2 of TMK 3-stage hashing pipeline:
...
void usage(char* argv0, int exit_rc) {
  ...
  fprintf(fp, "--output-features-file-name {x}\n");
...
int main(int argc, char** argv) {
  ...
  char* outputFeaturesFileName = nullptr;
  bool verbose = false;
  ...
  while ((argi < argc) && argv[argi][0] == '-') {
    ...
    } else if (!strcmp(flag, "--output-features-file-name")) {
      if ((argc - argi) < 1) {
        usage(argv[0], 1);
      }
      outputFeaturesFileName = argv[argi++];
  ...
  if (outputFeaturesFileName != nullptr) {
    outputFp =
        facebook::tmk::io::openFileOrDie(outputFeaturesFileName, "wb", argv[0]);
  }
  ...
  // ----------------------------------------------------------------
  if (!facebook::tmk::io::writeFrameFeaturesFileHeader(
          outputFp,
          algorithm,
          ...
          argv[0])) {
    fprintf(
        stderr, "%s: failed to write frame-features file header.\n", argv[0]);
  ...
  while (!feof(inputFp)) {
    ...
    if (!phasher->hashFrame(rawFrameBuffer.get(), frameFeature)) {
      fprintf(
          stderr,
    ...
    int write_rc = facebook::tmk::io::writeFloatVector(frameFeature, outputFp);
    if (write_rc <= 0) {
      fprintf(stderr, "%s: failed to write frame descriptor %d.\n", argv[0], 0);
      return (int)VideoHashingStatus::FATAL;
    }
  ...
  return (int)VideoHashingStatus::OK;
}
