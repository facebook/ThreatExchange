...
class JSONDescriptorFormatter implements DescriptorFormatter {
  ...
  public String format(ThreatDescriptor threatDescriptor, boolean includeIndicatorInOutput) {
    ...
    w.ifNotNullAdd("td_first_active", threatDescriptor.td_first_active);
    w.ifNotNullAdd("td_last_active", threatDescriptor.td_last_active);
    w.ifNotNullAdd("td_expired_on", threatDescriptor.td_expired_on);
