...
# AI Coding Policy
AI is moving very fast, but it's clear that they are helpful when used properly, and can speed up development. However, LLMs can also produce code that looks correct but has subtle bugs. AI can assist, but we ask that a human be behind every change, and to be able to vouch for a PR's contents. We apply the same standards for quality and correctness for every PR regardless of how they were produced. Contributors should be able to explain their changes, and we can't accept changes that contributors cannot adequately explain the reasoning for. If there are a few areas of your PR that was produced by LLM that you aren't sure how it works, we ask that you flag them either with code comments or with comments on the PR so we can give them extra scruitany in review. If you aren't sure how the majority of your changes work, we ask that you spend time before submitting the PR for review to learn how the underlying technologies work. If we end up in a situation where neither the contributor nor the reviewers know how the code works we won't be able to accept the PR! Additionally, if your summary or test plan was produced by AI, please review it for accuracy, and make sure all the steps in the test plan are accurate to what you personally tested. 
