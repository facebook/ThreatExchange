...
static bool test_8_uncached_against_slow_or_die() {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    if (uncached != slow) {
      fprintf(
          stderr,
          "8-bit hamming uncached(%02xx) = %d != slow %d\n",
          (unsigned)a,
          uncached,
          slow);
    }
  ...
  printf(
      "hamming test_8_uncached_against_slow_or_die %s\n", ok ? "PASS" : "FAIL");
  return ok;
}
...
static bool test_8_cached_against_slow_or_die() {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    if (cached != slow) {
      fprintf(
          stderr,
          "8-bit hamming cached(%02xx) = %d != slow %d\n",
          (unsigned)a,
          cached,
          slow);
    }
  ...
  printf(
      "hamming test_8_cached_against_slow_or_die %s\n", ok ? "PASS" : "FAIL");
  return ok;
}
...
static bool test_16_uncached_against_slow_or_die() {
  ...
  for (int i = 0; i < 65536; i++) {
    ...
    if (uncached != slow) {
      fprintf(
          stderr,
          "16-bit hamming uncached(%02xx) = %d != slow %d\n",
          (unsigned)a,
          uncached,
          slow);
    }
  ...
  printf(
      "hamming test_16_uncached_against_slow_or_die %s\n",
      ok ? "PASS" : "FAIL");
  return ok;
}
...
static bool test_16_cached_against_slow_or_die() {
  ...
  for (int i = 0; i < 65536; i++) {
    ...
    if (cached != slow) {
      fprintf(
          stderr,
          "16-bit hamming cached(%02xx) = %d != slow %d\n",
          (unsigned)a,
          cached,
          slow);
    }
  ...
  printf(
      "hamming test_16_cached_against_slow_or_die %s\n", ok ? "PASS" : "FAIL");
  return ok;
}
